...
async function main(){
    try{
        ...
        const judges = readJudgesFromExcel();

        console.log(`Read ${judges.length} judges from Excel.`);
    ...
    console.log(`Done. Upserted ${upsertedCount} written judges. Skipped ${skippedCount}.`);
    process.exit(0);
    } catch (error) {
        console.error('IMPORT WRITTEN JUDGES FAILED:', error);
        process.exit(1);
    }
}
